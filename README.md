...
## Reading layer `PROVCenso2010' from data source `/home/agrie/unidad-2-asignacion-1-esda-operaciones-con-atributos-plots-Estebaniarosario/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
proven17 %>%
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿La educación?')) %>%
  plot(breaks = 'jenks')
```
...
#Rótulos: nombres de las provincias
proven17 %>%
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿La educación?'), TOPONIMIA) %>%
  gather(variable, valor, -geom, -TOPONIMIA) %>% 
  ggplot() + aes(fill = valor) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds'), trans = 'log10') +
...
``` r
proven17 %>% st_drop_geometry() %>%
  dplyr::select(TOPONIMIA, contains('Principales problemas de su barrio o comunidad: ¿La educación?'))
##                 TOPONIMIA
## 1       DISTRITO NACIONAL
## 2                    AZUA
## 3                 BAORUCO
...
proven17 %>%
  ...
  group_by(TOPONIMIA) %>% 
  mutate(pct=round(valor/sum(valor)*100,2)) %>% dplyr::select(-valor) %>% 
  filter(variable %in% 'Principales problemas de su barrio o comunidad: ¿La educación?: No') %>% 
  ggplot() + aes(fill = pct) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds')) +
  geom_sf_text(aes(label=pct), check_overlap = T, size = 3)
...
``` r
proven17 %>% st_drop_geometry() %>% filter(ENLACE=='0319') %>% 
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿La educación?'))
##   Principales problemas de su barrio o comunidad: ¿La educación?: Si
## 1                                                                 15
##   Principales problemas de su barrio o comunidad: ¿La educación?: No
